...
bool Texture_loadFromFile(Window* window, Texture* texture, const char* filepath); // Loads an image from a specified path
bool Texture_loadPixelsFromFile(Window* window, Texture* texture, const char* filepath); // Loads the image into pixel buffer
bool Texture_loadFromPixels(Window* window, Texture* texture); // Creates an image from pre-loaded textures
bool Texture_loadFromRenderedText(Window* window, Texture* texture, TTF_Font* font, const char* text, SDL_Color text_color); // This function will return images based on the font and the color and obviosuly the text too duh!
...
void Texture_render(Window* window, Texture* texture, int x, int y, SDL_Rect *clip); // Renders texture at a given point and only a given part
void Texture_renderEx(Window* window, Texture* texture, int x, int y, SDL_Rect* clip, double angle, SDL_Point* center, SDL_RendererFlip flip); // Provides extra functionality while rendering
