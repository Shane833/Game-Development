...
bool Texture_createBlank(Texture * texture, Window * window, size_t width, size_t height, SDL_TextureAccess access)
{
	check(texture != NULL, "ERROR : Invalid Texture!");
	...
	// Create an uninitialized texture, now we can explictely define the access type
	// SDL_TEXTUREACCESS_TARGET - To render to a texture
	// SDL_TEXTUREACCESS_STREAMING - To lock and stream to a texture
	texture->texture = SDL_CreateTexture(window->renderer, SDL_PIXELFORMAT_RGBA8888, access, width, height);
...
// Sets itself as the render target
void Texture_setAsRenderTarget(Texture * texture, Window * window)
{
	check(window != NULL, "ERROR : Invalid Window!");
	check(texture != NULL, "ERROR : Invalid Texture!");
	check(texture->texture != NULL, "ERROR : No Texture Loaded!");

	// make itself the render target
	SDL_SetRenderTarget(window->renderer, texture->texture);
