...
const int DOT_VEL = 1; // reducing the velocity so the effect is more visible
...
void Dot_render(SDL_Renderer* renderer, Texture* texture, Dot* dot, int camX, int camY)
{
	check(dot != NULL, "Invalid Dot!");
	
	Texture_render(renderer, texture, dot->position.x - camX, dot->position.y - camY, NULL);
