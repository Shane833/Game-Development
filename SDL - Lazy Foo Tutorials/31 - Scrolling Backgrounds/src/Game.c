...
void update()
{
	...
	scrollingOffset -= 2;
	if( scrollingOffset < -Texture_getWidth(&bg_texture)){
		scrollingOffset = 0;
	}
}
...
void render()
{
	...
	Texture_render(renderer, &bg_texture, scrollingOffset, 0, NULL);
	Texture_render(renderer, &bg_texture, scrollingOffset + Texture_getWidth(&bg_texture), 0, NULL);
